...
toad trout turkey turtle weasel whale wolf wombat zebra'''.split()
    ...
    def info(self, info):
        ln=len(self.infStr)
        print(self.infStr[:-3], flush=True)
    ...
    def start(self):
        ...
        print('The word is: ', result, flush=True)
        success, i, missed = False, 0, []
        while i < len(self.pics)-1:
            print('Guess the word: ', end='', flush=True)
            guess,right = self.askAndEvaluate(word, result, missed)
            if guess == None:
                print('You\'ve already entered this character.', flush=True)
                continue
            ...
            print('Missed characters: ', missed, flush=True)
